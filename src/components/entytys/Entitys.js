...
function Entitys({ archiveNote, openModal, deleteNote, notesArray }) {
    ...
    const markup = 
            <div key={index} className="card d-4 a-2 bg-success text-white thatsMyNote" style={{width: 19 +rem}}>
             ...
             <p className="card-text">${note.text
               ...
               .replace(/</g, "&gt")}</p>
                ...
                <button id='${note.id}' className="btn btn-info" onclick={openModal}>Edit</button>
                <button id='${note.id}' className="btn btn-danger" onclick={deleteNote}>Delete</button>
                <button id='${note.id}' className="btn btn-success" onclick={archiveNote}>Archive</button>
                </div>
            </div>
